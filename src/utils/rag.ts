...
// Extend to include score
type ScoredDocument = Document & { score: number };

export async function getRelevantChunks(query: string): Promise<ScoredDocument[]>{
  try {
    ...
    const scoredDocs = (documents as Document[])
      .map((doc: Document): ScoredDocument => ({
        ...doc,
        score: cosineSimilarity(queryVec, doc.embedding),
      }))
      .filter(doc => doc.score > 0.1) // Filter out very low similarity scores
      .sort((a: ScoredDocument, b: ScoredDocument) => b.score - a.score);
